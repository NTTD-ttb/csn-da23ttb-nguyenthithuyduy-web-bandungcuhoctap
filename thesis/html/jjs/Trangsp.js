...
let currentPage = 1;
let dsDangHienThi = null; // null chưa lọc
...
function hienThiSanPham() {
    ...
    let dsDanhMuc = layDSSanPham().filter((sp) => sp.danhMuc === DANH_MUC);

    // nếu đã lọc → dùng kết quả lọc
    let ds = dsDangHienThi !== null ? dsDangHienThi : dsDanhMuc;

    // THÔNG BÁO KHÔNG CÓ SP
    if (ds.length === 0) {
        box.innerHTML = `
    ...
    dsTrang.forEach((sp) => {
        box.innerHTML += `
            <div class="col-6 col-md-3 san-pham">
...
document.querySelectorAll(".loc-gia").forEach((cb) => {
    cb.addEventListener("change", () => {
        const dsDanhMuc = layDSSanPham().filter((sp) => sp.danhMuc === DANH_MUC);

        const checked = document.querySelectorAll(".loc-gia:checked");

        if (checked.length === 0) {
            dsDangHienThi = null; // CHƯA LỌC
        } else {
            dsDangHienThi = dsDanhMuc.filter((sp) => {
...
document.querySelectorAll(".loc-gia").forEach((cb) => {
    cb.addEventListener("change", () => {
        giaDangChon = Array.from(document.querySelectorAll(".loc-gia:checked")).map((cb) => cb.value);
...
document.addEventListener("DOMContentLoaded", () => {
    hienThiSanPham(); // nếu trang có danh sách sản phẩm
    renderSanPhamNoiBat(); // sản phẩm nổi bật
});
